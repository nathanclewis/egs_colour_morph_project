...
## Model dataset as sf - CHANGED to California Albers (EPSG:3310) 
# Note: We removed the native_range st_filter so California data points can exist!
...
### Project across Greater Los Angeles Region -----
...
region_poly_albers <- st_as_sfc(la_bbox) %>% 
  st_transform("EPSG:3310")
...
### Extract average winter daily minimum temperature for LA -----
...
region_vect_wgs84 <- vect(st_as_sfc(la_bbox))
...
# Transform LA vector to landcover projection before cropping
la_lc_crs <- project(region_vect, crs(rast_lc))
rast_lc_cropped <- crop(rast_lc, ext(la_lc_crs))
...
pred_v <- vect(df_pred_melanism, geom = c("albers_x", "albers_y"), crs = "EPSG:3310")
...
df_regional_points <- df_projected %>% 
  st_filter(st_transform(st_as_sfc(la_bbox), "EPSG:3310"))
...
# Save plot
ggsave("Figures/melanism_projection_LA_June10_2026.tiff", proj_map, dpi = "retina")
