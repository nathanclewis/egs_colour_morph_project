...
## Model dataset as sf - UPDATED to Central Alberta Projection (UTM Zone 12N: EPSG:26912)
...
### Project across Greater Edmonton Area -----
...
region_poly_projected <- st_as_sfc(edmonton_bbox) %>% 
  st_transform("EPSG:26912")
...
### Extract average winter daily minimum temperature for Greater Edmonton -----
...
region_vect_wgs84 <- vect(st_as_sfc(edmonton_bbox))
...
# Transform Edmonton vector to landcover projection before cropping
edmonton_lc_crs <- project(region_vect, crs(rast_lc))
rast_lc_cropped <- crop(rast_lc, ext(edmonton_lc_crs))
...
# Set all as non-native (Edmonton is introduced range)
...
pred_v <- vect(df_pred_melanism, geom = c("albers_x", "albers_y"), crs = "EPSG:26912")
prob_raster <- rasterize(pred_v, pop_grid, field = "prob_melanic")


### Generate Plot Map -----

# Filter training points down to the local Edmonton bounding box
df_regional_points <- df_projected %>% 
  st_filter(st_transform(st_as_sfc(edmonton_bbox), "EPSG:26912"))
