...
mod_parsimonious <- glm(melanic_binary ~ pop_den_scaled + winter_temp_scaled + introduced + pop_den_scaled:introduced + winter_temp_scaled:introduced + winter_temp_scaled:pop_den_scaled + RAC,
...
### Extract human population density values (full range)
...
## Crop the stack to the MCP
winter_stack_cropped <- crop(tmin_stack, native_vect)
...
proj_map <- ggplot() +
  ...
  # Save plot
  ggsave("Figures/melanism_projection_w_points_May22_2026.tiff", proj_map, dpi = "retina")
